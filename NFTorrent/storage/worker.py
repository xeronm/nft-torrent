...
from .storage import TonStorageCli, TonStorageCliSettings
...
class TonStorageCliWorker(mp.Process):
    ...
    def run(self):
        ...
        try:
            ...
            logger.error("TonStorageCliWorker #{client_id:03d}: failed to init: {exc}",
                         client_id=self.client_id, exc=E)
            self.shutdown(11)
    ...
    async def main_loop(self):
        while not self.exit_event.is_set():
            try:
                ...
                logger.error("TonStorageCliWorker #{client_id:03d}: Subprocess error, output: {output}, {exc}",
                             client_id=self.client_id, output=E.output, exc=str(E))
                self.shutdown(10)
            except Exception:
                logger.error("TonStorageCliWorker #{client_id:03d}: Unhandled exception: {exc}",
                             client_id=self.client_id, exc=traceback.format_exc())
                raise
    ...
    async def report_state(self):
        while not self.exit_event.is_set():
            try:
                ...
                logger.debug("TonStorageCliWorker #{client_id:03d}: status notify is_alive: {is_alive}, peer_count: {peer_count}",  # noqa: E501
    ...
    def process_task(self, task: WorkerCliTask):
        ...
        if start_time < task.timeout:
            try:
                ...
                exception = E
                logger.warning("TonStorageCliWorker #{client_id:03d}: Got exception, method: {method}, task_id: {task_id}, args: {args}, kwargs: {kwargs}, exception: {exc}",  # noqa: E501
                               client_id=self.client_id, method=task.method, task_id=task.task_id,
                               args=task.args, kwargs=task.kwargs, exc=str(E))
            except Exception as E:
                exception = E
                logger.warning("TonStorageCliWorker #{client_id:03d}: Got unhandled exception, method: {method}, task_id: {task_id}, args: {args}, kwargs: {kwargs}, exception: {exc}",  # noqa: E501
                               client_id=self.client_id, method=task.method, task_id=task.task_id,
                               args=task.args, kwargs=task.kwargs, exc=E)
            else:
                logger.debug("TonStorageCliWorker #{client_id:03d}: Got result, method: {method}, task \"{task_id}\"",
                             client_id=self.client_id, method=task.method, task_id=task.task_id)
        else:
            exception = asyncio.TimeoutError()
            logger.warning("TonStorageCliWorker #{client_id:03d}: Received task after timeout, method: {method}, task_id: {task_id}",  # noqa: E501
                           client_id=self.client_id, method=task.method, task_id=task.task_id)
