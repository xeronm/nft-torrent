...
class TonStorageCli:
    ...
    def open(self):
        ...
        try:
            ...
            command = ' '.join(args)
            logger.debug("TonStorageCli #{client_id:03d}: Popen cmd: {cmd}",
                         client_id=self.client_id, cmd=command)
            ...
            logger.error("TonStorageCli #{client_id:03d}: Popen error - {exc}",
        ...
        try:
            ...
            logger.error("TonStorageCli #{client_id:03d}: Client communication error - {exc}",
                         client_id=self.client_id, exc=E)
            self.terminate()
            raise
    ...
    def close(self):
        ...
        try:
            ...
            returncode = -1
            self.terminate()
        logger.info("TonStorageCli #{client_id:03d}: Session closed, exitcode: {exitcode}",
                    client_id=self.client_id, exitcode=returncode)
        self._proc = None
    ...
    def is_alive(self):
        ...
        if return_code is not None:
            logger.error("TonStorageCli #{client_id:03d}: Session aborted, exitcode: {exitcode}",
                         client_id=self.client_id, exitcode=return_code)
            self.terminate()
        ...
        with selectors.DefaultSelector() as selector:
            ...
            while self._proc.poll() is None:
                ...
                if cur_time - st_time > timeout:
                    raise subprocess.TimeoutExpired(self.settings.storage_cli_binary, timeout,
                                                    output=None, stderr=output)
    ...
    def _cmd_command(self, command: str, as_json=True):
        try:
            ...
            if as_json:
                ...
                response = self._read_json(timeout=self.settings.request_timeout,
                                           match_error='Query error:')
            else:
    ...
    def cmd_create(self,
        ...
        if not path or not isinstance(path, str):
            raise ValueError('Invalid path value, must be non-empty string')
...
def __example():  # pragma: no cover
    ...
    out = cli.cmd_get('F70D2F7587DBDFD0928E1967A0B2783EC3ABD63846AEC3B055B4705AEF742871')
    while not out['torrent']['completed']:
        time.sleep(3)
        out = cli.cmd_get('F70D2F7587DBDFD0928E1967A0B2783EC3ABD63846AEC3B055B4705AEF742871')
    ...
    out = cli.cmd_create("C:/Work/ton-storage/file1.txt",
