...
class TestNodeJWTBearer(unittest.TestCase):
    ...
    def test_jwt_bearer(self):
        bearer_s10 = NodeJWTBearer(
            subject="192.168.1.10", jwt_algorithm="HS256", jwt_secret="0123456789", node_state=get_node_state
        )
        bearer_s20 = NodeJWTBearer(
            subject="192.168.1.20", jwt_algorithm="HS256", jwt_secret="0123456789", node_state=get_node_state
        )
        bearer_s30 = NodeJWTBearer(
            subject="192.168.1.30", jwt_algorithm="HS256", jwt_secret="0123456789", node_state=get_node_state
        )
