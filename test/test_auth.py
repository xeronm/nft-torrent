...
class TestNodeJWTBearer(unittest.TestCase):
    ...
    def test_jwt_bearer(self):
        bearer_s10 = NodeJWTBearer(subject='192.168.1.10',
                                   jwt_algorithm='HS256',
                                   jwt_secret='0123456789',
                                   node_state=get_node_state)
        bearer_s20 = NodeJWTBearer(subject='192.168.1.20',
                                   jwt_algorithm='HS256',
                                   jwt_secret='0123456789',
                                   node_state=get_node_state)
        bearer_s30 = NodeJWTBearer(subject='192.168.1.30',
                                   jwt_algorithm='HS256',
                                   jwt_secret='0123456789',
                                   node_state=get_node_state)
...
class TestContractAPIKeyCookie(unittest.TestCase):
    ...
    def test_auth_success(self):
        ...
        validated = None
        try:
            validated = contr.auth_verify(account=models.Account(**self.account),
                                          proof=models.TonProof(**self.proof)) or True
        except Exception as E:
            validated = str(E)
        self.assertEqual(validated, 'Signature has expired')

    @mock.patch('time.time', mock.MagicMock(return_value=1743608712 +
                                            ContractAPIKeyCookie.auth_payload_expires_timeout))
